...
<body class="bg-gray-50 text-gray-800 font-sans">
  ...
  <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-5xl w-full space-y-8 bg-white shadow-2xl rounded-2xl p-10">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <input type="date" name="publication_date" id="publication_date"
                class="mt-1 block w-full border rounded-lg p-2" />
              <span class="text-red-500 text-sm hidden" id="error-publication_date">Tanggal wajib diisi.</span>
          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3" id="categorySection">
              ...
              <label class="flex items-center gap-2 text-sm">
                <input type="checkbox" name="category_ids[]" value="{{ $category->id }}" class="kategori-checkbox" />
                {{ $category->name }}
              </label>
              @endforeach
            </div>
            <span class="text-red-500 text-sm hidden" id="error-kategori">Pilih minimal satu kategori.</span>
  ...
  document.getElementById('bookForm').addEventListener('submit', function (e) {
    ...
    fields.forEach(field => {
      ...
      // Sesuaikan id error dengan field, khusus publication_date pakai error-publication_date
    ...
    // Validasi kategori checkbox minimal 1
    const kategori = document.querySelectorAll('.kategori-checkbox');
    const kategoriChecked = [...kategori].some(checkbox => checkbox.checked);
    ...
    if (!valid) {
      e.preventDefault(); // cegah submit kalau validasi gagal
    }
  });
</script>
